...
cat("computing_funct ready. \n")
  # aggregate gruops to region level if needed
  # if(!(length(unique(data[,region]))==length(data[,region]))){
    # orig.data <- data
    # data <- aggregate(data[groups], by=data[region], FUN=sum)
  # }
...
cat("county_compute ready. \n")

  # compute "county_compute" for each county
  all_indices <- ddply(data, data[,as.character(county)], county_compute,
                       .parallel=FALSE)
    return(all_indices)
}
